...
abstract: >
  Short sprint performance is one of the most distinguishable and admired physical trait in sports. Short sprints have been modeled using the mono-exponential equation that involves two parameters: (1) maximum sprinting speed (MSS) and (2) relative acceleration (TAU). Short sprint performance data comes from two sources: (1) split times using timing gates and (2) radar gun. In this paper, we demonstrate how these data can be modeled using the \pkg{shorts} package and how timing issues (i.e., synchronization issues with the radar gun, or using the flying start with the timing gates), which are common in practical measurement settings, can introduce error in parameter estimates. Further, we demonstrated how different model definitions implemented in \pkg{shorts} package can alleviate these errors.
keywords:
...
Short sprints profiling is usually performed by: (1) measuring split times using timing gates (i.e., positioned at various distances, e.g., 5, 10, 20, 30, 40m), (2) getting a velocity trace using a radar gun. Estimation of MSS and TAU parameters from equation \@ref(eq:velocity-time) is performed in \pkg{shorts} package using non-linear least squares regression implemented in the `nls` function in the \pkg{base} R [@R-base] and `nlme` function in the \pkg{nlme} package [@R-nlme] for the mixed-effect models.
...
Rather than providing *apriori* time correction from the literature, \pkg{shorts} package provides an estimation of this parameter from the data provided, together with MSS and TAU. To estimate time correction, we use `shorts::model_using_splits_with_time_correction` function. Here is how we can estimate Jack parameters using either provided time correction (e.g., +0.3 and +0.5s) or estimated time correction:
...
Another model definition, which is a novel approach implemented in the \pkg{shorts} packages, is to utilize *distance correction*, besides time correction. Thus, equation \@ref(eq:time-distance) becomes:
...
This model is implemented in `shorts::model_using_splits_with_corrections` function. Below are the model estimates:
...
Let's utilize demonstrated functionalities of the \pkg{shorts} package using real-world data. The first dataset comes from Usain Bolt's performance from IAAF World Championship held in London, 2017, and the second dataset involve Jason Vescovi's sample data for 52 female soccer and field hockey athletes which comes with the \pkg{shorts} package (see `?vescovi`). 
