...
\Keywords{sprints, sports, modeling}
...
Short sprints profiling is usually performed by: (1) measuring split times using timing gates (i.e., positioned at various distances, e.g., 5, 10, 20, 30, 40m), and (2) getting a velocity trace using a radar gun. Estimation of MSS and TAU parameters from equation \eqref{eq:velocity-time} is performed in \pkg{shorts} package using non-linear least squares regression implemented in the \texttt{nls} function in the \pkg{base} R \citep{R-base} and \texttt{nlme} function in the \pkg{nlme} package \citep{R-nlme} for the mixed-effect models.
...
Here is a plot of observed and model predicted split times, with the help of the \pkg{ggplot2} package \citep{R-ggplot2}:
...
Plotting the model predictions can be done once we convert data from wide to long with the help of \pkg{dplyr} \citep{R-dplyr}, \pkg{tidyr} \citep{R-tidyr}, and \pkg{tidyverse} \citep{R-tidyverse} packages:
...
Another valuable addition for sport scientists and coaches is the ability to determine the distances and times where 90\% of maximum sprinting speed is reached, or where peak power is within 90\% range. To identify these values, \pkg{shorts} package comes with \texttt{shorts::find\_} family of functions:
...
Mixed-effects model using radar data is done using \texttt{shorts::mixed\_model\_using\_radar} function. To perform mixed model, let's load data that comes with \pkg{shorts} package.
...
Rather than providing \emph{apriori} time correction from the literature, \pkg{shorts} package provides estimation of this parameter from the data provide, together with MSS and TAU. To estimate time correction, we use \texttt{shorts::model\_using\_splits\_with\_time\_correction} function. Here is how we can estimate Jack parameters using either provided time correction (e.g., +0.3 and +0.5s) or estimated time correction:

\begin{CodeChunk}
\begin{CodeInput}
R> jack_profile_fixed_time_short <- shorts::model_using_splits(
+   distance = split_times$distance,
+   time = split_times$jack_time,
+   time_correction = 0.3
+ )
R> 
R> jack_profile_fixed_time_long <- shorts::model_using_splits(
+   distance = split_times$distance,
+   time = split_times$jack_time,
+   time_correction = 0.5
+ )
R> 
R> jack_profile_time_estimated <- shorts::model_using_splits_with_time_correction(
+   distance = split_times$distance,
+   time = split_times$jack_time
+ )
R> 
...
Another model definition, which is novel approach implemented in the \pkg{shorts} packages, is to ulitize \emph{distance correction}, besides time correction. Thus, equation \eqref{eq:time-distance} gets another form:
...
\begin{center}\includegraphics[width=1\linewidth]{paper_files/figure-latex/unnamed-chunk-61-1} \end{center}
